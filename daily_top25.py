...
def get_today_predictions():
    ...
    try:
        ...
        result = supabase.table("predictions")            .select("*")            .gte("match_date", yesterday.isoformat())            .lte("match_date", in30days.isoformat() + "T23:59:59")            .gte("rec_confidence", MIN_CONF_TOP)            .is_("prediction_correct", "null")            .not_.ilike("league_name", "%Friendlies%")            .not_.ilike("league_name", "%Tercera%")            .not_.ilike("league_name", "%Primera C%")            .not_.ilike("league_name", "%Paulista%")            .not_.ilike("league_name", "%Serie D%")            .not_.ilike("league_name", "%Tournoi%")            .not_.ilike("league_name", "%Tasmania%")            .not_.ilike("league_name", "%Baiano%")            .not_.ilike("league_name", "%Mineiro%")            .not_.ilike("league_name", "%Catarinense%")            .not_.ilike("league_name", "%Landesliga%")            .not_.ilike("league_name", "%Oberliga%")            .not_.ilike("league_name", "%Second League%")            .not_.ilike("league_name", "%USL League Two%")            .not_.ilike("league_name", "%NPL%")            .not_.ilike("league_name", "%Queensland%")            .not_.ilike("league_name", "%Capital Territory%")            .not_.ilike("league_name", "%4. liga%")            .not_.ilike("league_name", "%Ligi kuu%")            .not_.ilike("league_name", "%MLS Next Pro%")            .not_.ilike("league_name", "%Maurice Revello%")            .not_.ilike("league_name", "%Toulon%")            .not_.ilike("league_name", "%U20%")            .not_.ilike("league_name", "%U21%")            .not_.ilike("league_name", "%U23%")            .order("rec_confidence", desc=True)            .execute()
        data = result.data or []
