...
def fetch_team_players(team_id, team_name):
    ...
    for attempt in range(3):  # retry 3 fois
        try:
            ...
            print(f"Erreur {r.status_code} pour {team_name} (tentative {attempt+1})")
            time.sleep(2)
...
def save_players(players, team_name, team_sofascore_id):
    ...
    for item in players:
        ...
        data = {
            ...
            "jersey_number": str(
                p.get("jerseyNumber") or
                p.get("shirtNumber") or ""
            ),
            "date_of_birth": dob,
            "height": p.get("height"),
            ...
            "nationality": (
                ...
                if p.get("country") else ""
            ),
            "market_value": (
                p.get("proposedMarketValueRaw", {}).get("value")
                if p.get("proposedMarketValueRaw") else None
            ),
